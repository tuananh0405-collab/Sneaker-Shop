import { View, Text, FlatList, Image, TouchableOpacity, ActivityIndicator, Alert, Dimensions, Platform, ScrollView } from "react-native";
import React from "react";
import { SafeAreaView } from "react-native-safe-area-context";
import { useGetCartQuery, useUpdateCartMutation, useRemoveFromCartMutation } from "@/redux/api/cartApiSlice";
    ...
    <View className="flex-1 bg-white">
      <ScrollView showsVerticalScrollIndicator={false} contentContainerClassName="pb-32 bg-white ">
        <View className="relative w-full" style={{ height: windowHeight / 2 }}>
          <Image source={images.japan} className="size-full" style={{ height: 450 }} resizeMode="cover" />
        ...
        <View className=" px-4">
          ...
          {cart?.products.length === 0 ? (
            <View className="flex-1 justify-center items-center">
              <Text className="text-lg font-rubik-medium text-gray-500">Your cart is empty</Text>
            </View>
          ) : (
            <FlatList
              data={cart?.products}
              ...
              renderItem={({ item }) => (
                <View className="flex flex-row items-center justify-between mb-4 border-b border-gray-300 pb-2">
                  <Image source={{ uri: item.image[0] }} className="w-16 h-16 rounded-md" />
                  ...
                  <View className="flex-1 ml-4">
                    ...
                    <Text className="text-primary-300 font-rubik-bold">${item.price}</Text>
                  </View>

                  <View className="flex flex-row items-center">
                    <TouchableOpacity
                      onPress={() =>
                        handleUpdateQuantity(item.product, item.size, item.color, item.quantity - 1)
                      }
                    ...
                    <TouchableOpacity
                      onPress={() => handleRemoveItem(item.product, item.size, item.color)}
                      className="ml-3"
                    >
      ...
      <View className="absolute bg-white bottom-0 w-full rounded-t-2xl border-t border-r border-l border-primary-200 p-7">
        <View className="flex flex-row items-center justify-between gap-10">
          ...
          <TouchableOpacity className="flex-1 flex flex-row items-center justify-center bg-primary-300 py-3 rounded-full shadow-md shadow-zinc-400">
            <Text className="text-white text-lg text-center font-rubik-bold">Checkout Now</Text>
          </TouchableOpacity>
        </View>
      </View>
    </View>
